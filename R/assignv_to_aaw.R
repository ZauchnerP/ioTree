...
#' The name of the variable that should be assigned to all_apps_wide.
#' @param newvar Character.
#' The name of the newly created variable in the all_apps_wide data frame.
#' @param resafter Character.
#' The name of the variable that precedes the new variable.
#' If NULL, the new variable will be placed at the end of the data frame.
...
assignv_to_aaw <- function(oTree,
  ...
  if (app == "Chats" ||
    ...
    stop(paste0("This function does not work with ",
                app, "!"))
  }
  ...
  if ("all_apps_wide" %in% names(oTree)) {
    if (nrow(oTree[[app]]) != nrow(oTree$all_apps_wide)) {
      warning(
        paste0("New variable is created. However, there is an unequal ",
    ...
    stop(paste0("There is no \"all_apps_wide\" in your oTree list of ",
                "data frames!"))
  }
