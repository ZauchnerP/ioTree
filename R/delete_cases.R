...
#'
#' Caution 1: This function does not delete the cases from the original
#' CSV and Excel files!
#'
...
#' @param omit Logical. TRUE if the deleted cases should not be added to
#' the information on deleted cases in $info$deleted_cases.
#' @param info Logical. TRUE if a brief information on the case deletion
...
delete_cases <- function(oTree,
  ...
  if (omit == FALSE && length(del_participant_code_aaw) > 0) {
  ...
  } else if (omit == FALSE && length(del_participant_code_aaw) == 0) {
