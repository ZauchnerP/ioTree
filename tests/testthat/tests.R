...
library(ioTree)          # load our package

# Set data folder first
withr::with_dir(system.file(
  file.path("extdata"),  # Make sure the file path is read on windows and ubuntu
  package = "ioTree"), {
...
testthat::test_that("Import - bot files", {
  ...
  testthat::expect_equal(
    unique(otree_bot$all_apps_wide$participant._is_bot), 1)
})
...
testthat::test_that("Import - Excel", {
  ...
  test1 <- length(otree2$info$imported_files)  == 7
  # Don't do that because there are also faulty files: length(file_names) + 2
  testthat::expect_true(test1)
})
...
testthat::test_that("Import - delete dropouts", {
  ...
  otree2 <- delete_dropouts(otree1, final_apps = "survey", info = TRUE)
...
testthat::test_that("Import - empty rows check", {
  ...
  testthat::expect_gt(nrow(otree2$survey),
                      nrow(otree3$survey))
  testthat::expect_equal(nrow(otree2$Time),
                      nrow(otree3$Time))
  testthat::expect_equal(nrow(otree2$Chats),
                         nrow(otree3$Chats))
})
...
testthat::test_that("Import - with 2 paths", {
  ...
  otree1 <- import_otree(
    del_empty = TRUE,
    path = c(file.path(".", "exp_data_2.2.4")),
    onlybots = FALSE,
    csv = TRUE,
    info = TRUE)  # No warning here

  testthat::expect_warning(
    otree2 <- import_otree(
      del_empty = TRUE,
      path = file.path(".", "exp_data_5.4.0"),
      onlybots = FALSE,
      csv = TRUE,
      info = TRUE), " stored all_apps_wide globally but also room-specific")

  testthat::expect_warning(
    otree3 <- import_otree(
      del_empty = TRUE,
      path = c(file.path(".", "exp_data_2.2.4"),
               file.path(".", "exp_data_5.4.0")),
      onlybots = FALSE,
      csv = TRUE,
      info = TRUE
...
testthat::test_that("Import - with file_names(path included)", {
  ...
  testthat::expect_true(test1)
  test2 <- length(otree2$info$imported_files) == 3
...
testthat::test_that("Import - only time", {
  ...
  otree2 <- import_otree(
    ...
    path = system.file(
      file.path("extdata"),
      package = "ioTree"),
    file_names = "exp_data_5.4.0/PageTimes-2023-05-16.csv",
    onlybots = FALSE,
    csv = TRUE,
    info = TRUE)
...
testthat::test_that("Import - only Chats not faulty", {
  ...
  test2 <- length(otree2$info$imported_files) == 1
...
testthat::test_that("Import - with some faulty file_names", {
  ...
  # Test
  test1 <- ("initial_n" %in% names(otree2$info))
  test2 <- length(otree2$info$imported_files) == 4
  test3 <- "dictator" %in% names(otree2)
...
testthat::test_that("Delete duplicate", {
  ...
  before_aaw <- nrow(otree2$all_apps_wide)
  before_dictator <- nrow(otree2$dictator)
  ...
  # Test
  after_aaw <- nrow(otree2$all_apps_wide)
  after_dictator <- nrow(otree2$dictator)
  after_n <- otree2$info$initial_n
  testthat::expect_gt(before_aaw, after_aaw)
  testthat::expect_gt(before_dictator, after_dictator)
...
testthat::test_that("Delete duplicate - time and chat", {
  ...
  before_aaw <- nrow(otree2$all_apps_wide)
  before_dictator <- nrow(otree2$dictator)
  before_survey <- nrow(otree2$survey)
  before_time <- nrow(otree2$Time)
  before_chat <- nrow(otree2$Chat)
  before_n <- otree2$info$initial_n
...
testthat::test_that("Delete cases - new oTree", {
  ...
  after_aaw <- nrow(otree2$all_apps_wide)
  after_dictator <- nrow(otree2$dictator)
  after_survey <- nrow(otree2$survey)
  after_time <- nrow(otree2$Time)
  after_chat <- nrow(otree2$Chat)

  # Diff
  diff_aaw <- before_aaw - after_aaw
  diff_dictator <- before_dictator - after_dictator
  diff_survey <- before_survey - after_survey
  diff_time <- before_time - after_time
  diff_chat <- before_chat - after_chat

  # Test difference
  testthat::expect_gt(diff_aaw, 0)
  testthat::expect_gt(diff_dictator, 0)
  testthat::expect_gt(diff_survey, 0)
  testthat::expect_gt(diff_time, 0)
  testthat::expect_equal(diff_chat, 0)

  # Test if not all cases were deleted!
  testthat::expect_gt(after_aaw, 0)
  testthat::expect_gt(after_dictator, 0)
  testthat::expect_gt(after_survey, 0)
  testthat::expect_gt(after_time, 0)
  testthat::expect_gt(after_chat, 0)

  # Test if deleted people are really deleted
  test0 <- length(otree2$info[["deleted_cases"]][["codes"]]) == 1
...
testthat::test_that("Delete cases - new oTree, one random data frame", {
  ...
  after_time <- nrow(otree2$Time)
  after_chat <- nrow(otree2$Chat)
  after_random <- nrow(otree2$random_dataframe)
  ...
  diff_time <- before_time - after_time
  diff_chat <- before_chat - after_chat
  diff_random <- before_random - after_random
  ...
  testthat::expect_gt(diff_time, 0)
  testthat::expect_equal(diff_chat, 0)
  testthat::expect_equal(diff_random, 0)
  ...
  testthat::expect_gt(after_time, 0)
  testthat::expect_gt(after_chat, 0)
  testthat::expect_gt(after_random, 0)
...
testthat::test_that("Delete cases - old and new otree", {
  ...
  person1 <- otree2$Time$participant_code[
    !is.na(otree2$Time$participant_code)][1]
  person2 <- otree2$Time$participant__code[
    !is.na(otree2$Time$participant__code)][1]
  ...
  test1 <- length(otree2$info[["deleted_cases"]][["codes"]]) == 2
  testthat::expect_true(test1)
...
testthat::test_that("Delete cases - more people ", {
  # Prepare data
  otree2 <- otree_5_4_0
  person <- otree2$all_apps_wide$participant.code[c(1, 2, 3, 4)]
...
testthat::test_that("Delete cases - more people - one not there", {
  # Prepare data
  otree2 <- otree_5_4_0
  person <- c(otree2$all_apps_wide$participant.code[c(1, 2, 3, 4)],
                   "notthere")
...
testthat::test_that("Delete cases - person not in aaw", {
  # Prepare data
  otree1 <- otree_5_4_0
  person <- otree1$all_apps_wide$participant.code[1]
  otree1$all_apps_wide <- tail(otree1$all_apps_wide, -1)

  # Before
  before_aaw <- nrow(otree1$all_apps_wide)
  before_dictator <- nrow(otree1$dictator)
  before_survey <- nrow(otree1$survey)
  before_time <- nrow(otree1$Time)
  before_chat <- nrow(otree1$Chat)
  ...
  # Test difference
  testthat::expect_equal(diff_aaw, 0)
  testthat::expect_gt(diff_dictator, 0)
  testthat::expect_gt(diff_survey, 0)
  testthat::expect_gt(diff_time, 0)
  testthat::expect_equal(diff_chat, 0)
  ...
  testthat::expect_equal(test5a, test5b)
...
testthat::test_that("Delete cases - plabels", {
  ...
  # Test if deleted people are really deleted
  test0 <- all(person_codes %in% otree2$info[["deleted_cases"]][["codes"]])
  test1 <-
    !(otree2$info$deleted_cases$codes %in%
        otree2$all_apps_wide$participant.code)
  test2 <-
...
testthat::test_that("Delete cases - old", {
  ...
  testthat::expect_gt(diff_survey, 0)
  testthat::expect_gt(diff_time, 0)
  testthat::expect_equal(diff_chat, 0) # Equal zero!!!
...
testthat::test_that("Delete cases - saved vars", {
  ...
  test1 <- length(otree2$info[["deleted_cases"]][["codes"]]) == 1
  testthat::expect_true(test1)
...
testthat::test_that("Delete cases - old", {
  ...
  if (check_time == TRUE) {
    ...
    testthat::expect_equal(diff_time, 0)
  }
  testthat::expect_equal(diff_chat, 0)
...
testthat::test_that("Delete sessions - old ", {
  ...
  session <- session[session$Freq >= 4, 1]
  ...
  testthat::expect_gt(diff_survey, 0)
  testthat::expect_gt(diff_time, 0)
  testthat::expect_gt(diff_chat, 0)
...
testthat::test_that("Delete sessions - no cases", {
  ...
  # Test
  testthat::expect_equal(diff_aaw, 0)
  testthat::expect_equal(diff_dictator, 0)
  testthat::expect_equal(diff_survey, 0)
  testthat::expect_equal(diff_time, 0)
  testthat::expect_equal(diff_chat, 0)
...
testthat::test_that("Delete sessions - without aaw", {
  # Prepare data (take a session with more people so there is a chat in it)
  otree2 <- otree_5_4_0
  session <- as.data.frame(table(otree2$all_apps_wide$session.code))
  session <- session[session$Freq >= 4, 1]
  part <- otree2$all_apps_wide$participant.code[
  ...
  testthat::expect_gt(diff_survey, 0)
  testthat::expect_gt(diff_time, 0)
  testthat::expect_gt(diff_chat, 0)  # here greater 0 because all people
  ...
  test4 <-
    !(otree2$info$deleted_cases$codes %in%
        otree2$Time$participant_code)
  test5 <- all(!(otree2$Chats$participant_code %in%
                   otree2$info$deleted_cases$codes))
...
testthat::test_that("Delete sessions - old - warning Chat", {
  # Prepare data
  otree2 <- otree_2_2_4
  session <- unique(otree2$Chats$participant__session__code)[1]
  ...
  test4 <-
    !(otree2$info$deleted_cases$codes %in%
        otree2$Time$participant__code)
  test5 <- all(!(otree2$Chats$participant__code %in%
                   otree2$info$deleted_cases$codes))
...
testthat::test_that("Show dropouts - new oTree, final_apps, one random df", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$random_dataframe <- data.frame(
    a = c(1, 2, 3),
...
testthat::test_that("Show dropouts - 2", {
  ...
  person <- otree2$all_apps_wide$participant.code[
    otree2$all_apps_wide$participant._current_app_name == "survey"][1]
  otree2$all_apps_wide$participant._current_app_name[
    otree2$all_apps_wide$participant._current_app_name == "survey"][1] <-
    "othername"
...
testthat::test_that("Show dropouts - saved vars", {
  ...
  # Run function
  output <- show_dropouts(otree2, "survey",
                          saved_vars = "dictator.1.group.id_in_subsession")
  ...
  testthat::expect_warning(
    output <-
      show_dropouts(otree2,
        final_apps = "survey",
        saved_vars = "dictator.1.group.id_in_subsession"), "is ignored.")

  # Test
  # Test if all data frames and other vectors/values are there
  test1 <- "full" %in% names(output)
  test2 <- "unique" %in% names(output)
  test3 <- !("all_end" %in% names(output)) # all_end is only shown if aaw exist
  test4 <- "codes" %in% names(output)
  test5 <- "count" %in% names(output)
  testthat::expect_true(all(c(test1, test2, test3,
                              test4, test5)))
...
testthat::test_that("Delete dropouts 2 - inconsistent yes", {
  ...
  person <- otree1$all_apps_wide$participant.code[
    otree1$all_apps_wide$participant._current_app_name == "survey"][1]
...
testthat::test_that("Delete dropouts 2 - inconsistent check", {
  ...
  options(mypkg.connection = my_file)
  ...
  cat_output_displayed <-
    any(grepl("At least one participant in the dropout list", output))
  testthat::expect_true(cat_output_displayed)

  # Reset the mypkg.connection option to stdin() and close the text connection
...
testthat::test_that("several deletions  - version 1", {
  ...
  # Session delete
  session <- as.data.frame(table(otree2$all_apps_wide$session.code))
  session <- session[session$Freq >= 2, 1]
  otree2 <- delete_sessions(otree2,
                            scodes = session,
                            reason = "Only tests")
...
testthat::test_that("several deletions - version 2", {
  ...
  # Session delete
  otree2$all_apps_wide$session.code[1]
  otree2 <- delete_sessions(otree2,
                            scodes = otree2$all_apps_wide$session.code[1],
                            reason = "Only tests")
...
testthat::test_that("several deletions - version 3", {
  ...
  # Delete cases
  person <- otree2$all_apps_wide$participant.code[c(1, 2)]
  otree2 <- delete_cases(otree2, person,
                         reason = "Upon request")
  # Test
  test0 <- !("" %in% otree2$info$deleted_cases$unique$reason)
...
testthat::test_that("Make IDs - from_var, gmake = TRUE", {
  ...
  test3 <- all(
    c(1:(length(otree2$dictator$group_id) / 3 / 2)) %in%
  ...
  otree2$dictator <- otree2$dictator[order(otree2$dictator$session_id,
                                           otree2$dictator$group_id), ]
  test1 <- all(diff(otree2$dictator$group_id) >= 0)

  # Test if participant_ids are increasing with group_ids
  otree2$dictator <- otree2$dictator[order(otree2$dictator$group_id,
...
testthat::test_that("Make IDs - two sessions na", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$all_apps_wide$session.code[c(1, 2, 3)] <- NA
  order_aaw_before <- otree2$all_apps_wide$participant.code
  order_dictator_before <- otree2$dictator$participant.code
...
testthat::test_that("Make IDs - from_var, other starts", {
  ...
  test1 <- min(otree2$all_apps_wide$group_id) == 4
  test2 <- min(otree2$all_apps_wide$session_id) == 3
  test3 <- min(otree2$all_apps_wide$participant_id) == 8
  testthat::expect_true(all(c(test1, test2, test3)))
...
testthat::test_that("Make IDs - from_var, dictator.1.group.id_in_subsession", {
  # Prepare data
  otree2 <- otree_5_4_0
...
testthat::test_that("Make IDs - all the same", {
  ...
  # Test if all IDs are there
  test1 <- all(c(1:max_part) %in%
                 otree2$all_apps_wide$participant_id)
  test2 <- all(c(1:max_session) %in%
                 otree2$all_apps_wide$session_id)
  test3 <- all(
    c(1:max_group) %in%
      otree2$dictator$group_id)
...
testthat::test_that("Make IDs - no time start", {
  ...
  test1 <- !any(duplicated(numbers))
  test2 <- !any(duplicated(strings))
...
testthat::test_that("Make IDs - from_var - first constant", {
  ...
  # Test
  test1 <- all(otree2$all_apps_wide$group_id == otree2$all_apps_wide$session_id)
  testthat::expect_true(test1)
  ...
  test2 <- all(seq_along(unique(otree2$all_apps_wide$session.code)) %in%
                 otree2$all_apps_wide$session_id)
  test3 <- otree2$all_apps_wide$session_id == otree2$all_apps_wide$group_id
  testthat::expect_true(all(c(test1, test2, test3)))
})
...
testthat::test_that("Make IDs - from_app", {
  ...
  testthat::expect_equal(length(otree2$all_apps_wide$participant_id),
                         length(test_participant))

  # Test if variables exist / don't exist
  test1 <- !is.null(otree2$dictator$group_id)
  test2 <- !is.null(otree2$dictator$session_id)
  testthat::expect_equal(test1, TRUE)
  testthat::expect_equal(test2, TRUE)
...
testthat::test_that("Make IDs - one participant not in from_app", {
  ...
  otree2 <- otree_5_4_0
  otree2 <- delete_duplicate(otree2)
  delsomedictators <- unique(otree2$dictator$participant.code)[c(1)]
  ...
  # Test if all IDs are there
  test1 <- all(c(1:(length(unique(otree2$all_apps_wide$participant.code)) - 1))
...
testthat::test_that("Make IDs - from_var constant", {
  ...
  # Test
  testthat::expect_equal(otree2$group_id, otree2$session_id)
  ...
  test2 <- all(seq_along(unique(otree2$all_apps_wide$session.code)) %in%
                 otree2$all_apps_wide$session_id)
  test3 <- all(otree2$all_apps_wide$session_id == otree2$all_apps_wide$group_id)
  testthat::expect_true(all(c(test1, test2, test3)))
...
testthat::test_that("Make IDs - others more cases than from-app", {
  ...
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$dictator <- otree2$dictator[c(7:nrow(otree2$dictator)), ]
  order_aaw_before <- otree2$all_apps_wide$participant.code
  order_dictator_before <- otree2$dictator$participant.code
  ...
  test1 <- all(c(1:(length(otree2$all_apps_wide$participant.code) -
                      sum(is.na(otree2$all_apps_wide$participant_id)))) %in%
                 otree2$all_apps_wide$participant_id)
  test2 <- all(c(1:(length(unique(otree2$all_apps_wide$session.code)) - 1)) %in%
                 otree2$all_apps_wide$session_id)
  test3 <- all(
    c(1:(length(otree2$dictator$group_id) / 3 / 2)) %in%
      otree2$dictator$group_id)
...
testthat::test_that("Make IDs - from_app more cases than all apps wide", {
  # Prepare data
  otree2 <- otree_5_4_0
  delperson <- otree2$dictator$participant.code[1]

  otree2$all_apps_wide <-
    otree2$all_apps_wide[otree2$all_apps_wide$participant.code != delperson, ]
  order_aaw_before <- otree2$all_apps_wide$participant.code
  order_dictator_before <- otree2$dictator$participant.code
  ...
  # Test if variables are there
  test3 <- !is.null(otree2$dictator$group_id)
  test4 <- !is.null(otree2$dictator$session_id)
  test5 <- !is.null(otree2$dictator$participant_id)
  testthat::expect_true(all(c(test3, test4, test5)))

  # Test if values are not there twice
  test <- length(otree2$all_apps_wide$participant_id) ==
    length(unique(otree2$all_apps_wide$participant_id))
  testthat::expect_true(test)

  # Test if all IDs are there
  test6 <- all(c(2:(length(otree2$all_apps_wide$participant.code))) %in%
                 otree2$all_apps_wide$participant_id)
  test7 <- all(seq_along(unique(otree2$all_apps_wide$session.code)) %in%
                 otree2$all_apps_wide$session_id)
  test8 <- all(
    c(1:(length(otree2$dictator$group_id) / 3 / 2)) %in%
      otree2$dictator$group_id)
  ...
  otree2$dictator <- otree2$dictator[order(otree2$dictator$session_id,
                                           otree2$dictator$group_id), ]
  test9 <- all(diff(otree2$dictator$group_id) >= 0)

  # Test if participant_ids are increasing with group_ids
  otree2$dictator <- otree2$dictator[order(otree2$dictator$group_id,
                                           otree2$dictator$participant_id), ]
  test10 <- all(diff(otree2$dictator$participant_id) >= 0)
  testthat::expect_true(all(c(test9, test10)))
})
...
testthat::test_that("Make IDs (w) - from_var constant", {
  ...
  # Test
  testthat::expect_equal(otree2$all_apps_wide$group_id,
                         otree2$all_apps_wide$session_id)

  # Test if variables exist / don't exist
  test0 <- !is.null(otree2$all_apps_wide$group_id)
  test1 <- !is.null(otree2$all_apps_wide$participant_id)
  testthat::expect_true(all(c(test0, test1)))
  ...
  test3 <- all(seq_along(unique(otree2$all_apps_wide$session.code)) %in%
                 otree2$all_apps_wide$group_id)  # Because group is constant
  testthat::expect_true(all(c(test1, test2, test3)))
...
testthat::test_that("Make IDs (w) - constant group variable", {
  ...
  test7 <- otree2$all_apps_wide$group_id == otree2$all_apps_wide$session_id
...
testthat::test_that("Make IDs (w) - from_var - no participant code variable", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2 <- delete_duplicate(otree2)
...
testthat::test_that("Make IDs (e) - empty rows yes", {
  ...
  # Reset the mypkg.connection option to stdin()
  # and close the text connection
  options(mypkg.connection = stdin())
  close(my_file)
})
...
testthat::test_that("Make IDs (e) - participant_code and participant__code", {
  # Prepare data
  otree2 <- otree_all
...
testthat::test_that("Make IDs (e) - NA in participant__code", {
  # Prepare data
  otree2 <- otree_2_2_4
  otree2$Chats$participant__code[1] <- NA # leaves only nas in participant_code
...
testthat::test_that("Make IDs (e) - NA in participant_code", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$Chats$participant_code[1] <- NA
...
testthat::test_that("Make IDs (e) - from_var not found, session codes Na", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$all_apps_wide$session.code[c(1, 5, 3)] <- NA
...
testthat::test_that("Make IDs (e) - chat_warning", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$Chats$participant_code <- 1
...
testthat::test_that("Make IDs - from_app (old)", {
  ...
  testthat::expect_equal(max(otree2$dictator$group_id), 6)
  testthat::expect_equal(max(otree2$dictator$session_id), 2)
...
testthat::test_that("extime - all oTree", {
  ...
  test5 <- round(output$min_duration, 2) ==
    round(min(output$single_durations$duration), 2)
  test6 <- round(output$max_duration, 2) ==
    round(max(output$single_durations$duration), 2)
  test7 <- round(output$mean_duration, 2) ==
    round(mean(output$single_durations$duration), 2)
  testthat::expect_true(all(c(test1, test2, test3, test4,
                              test5, test6, test7)))
})
...
testthat::test_that("extime - new oTree", {
  ...
  test6 <- round(mean(output$single_durations$duration),
                 digits = 2) == output$mean_duration
...
testthat::test_that("extime - 3 digits", {
  ...
  # Run function
  output <- extime(otree2, digits = 3)

  # Test
  test1 <- round(mean(output$single_durations$duration), digits = 3) ==
    output$mean_duration
  test2 <- round(min(output$single_durations$duration), digits = 3) ==
    output$min_duration
  test3 <- round(max(output$single_durations$duration), digits = 3) ==
    output$max_duration
  # Test if the values and the single_durations align
  test5 <- round(output$min_duration, 2) ==
    round(min(output$single_durations$duration), 2)
...
testthat::test_that("extime - new", {
  # Prepare data
  otree2 <- otree_5_4_0

  # Run function
  output <- extime(otree2, startat = 1)
...
testthat::test_that("extime - for a specific group", {
  ...
  # Run
  output <- extime(otree2, group_id = 1)
...
testthat::test_that("extime - experiment just one page", {
  ...
  test5 <- grepl("the experiment only has one page", output$messages)
  test6 <- is.vector(output$only_one_page)

  # Test if the values and the single_durations align
  test7 <- round(output$min_duration, 2) ==
    round(min(output$single_durations$duration), 2)
  test8 <- round(output$max_duration, 2) ==
    round(max(output$single_durations$duration), 2)
  test9 <- round(output$mean_duration, 2) ==
    round(mean(output$single_durations$duration), 2)
  testthat::expect_true(all(c(test1, test2, test3, test4,
...
testthat::test_that("extime - one person only one page", {
  ...
  person <- aggregate(otree2$Time,
                      ...
                      FUN = "max")
  person <- unlist(person[person$page_index == 1, ][1])
...
testthat::test_that("extime - startat comparison - comparison real und 1", {
  # Prepare data
  otree2 <- otree_5_4_0
  ...
  # Der unterschied zu start1 ist, dass bei start1 nicht kommazahlen kommen.
  # Das liegt daran, dass "real" eine genauere Anfangszeit hat!!!
...
testthat::test_that("extime - for a specific individual", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2 <- make_ids(otree2, gmake = TRUE, from_app = "dictator")
  person <- otree2$all_apps_wide$participant.code[1]
  ...
  testthat::expect_equal(output, round(duration, 2))
})
...
testthat::test_that("extime - for a specific individual (real)", {
  ...
  otree2 <- otree_5_4_0
  otree2 <- make_ids(otree2, gmake = TRUE, from_app = "dictator")
  person1 <- unique(otree2$Time$participant_code)[1]
...
testthat::test_that("extime (w) - warning only one page for old specific", {
  # Prepare data
  otree2 <- otree_2_2_4
  person <- aggregate(otree2$Time,
                      list(person = otree2$Time$participant__code),
...
testthat::test_that("extime (e) - old specific: TIME for one empty", {
  # Prepare data
  otree2 <- otree_2_2_4
  person <- otree2$all_apps_wide$participant.code[2]
...
testthat::test_that("extime (e) - startat: new too low", {
  # Prepare data
  otree2 <- otree_2_2_4
  otree2$Time <- otree2$Time[otree2$Time$page_index > 2, ]
...
testthat::test_that("extime (e) - startat: new too low", {
  ...
  otree2 <- otree_2_2_4
  otree2$Time <- otree2$Time[otree2$Time$page_index > 2, ]
  person <- otree2$Time$participant__code[otree2$Time$page_index > 5][1]
...
testthat::test_that("extime (e) - variable group_id not there", {
  ...
  testthat::expect_error(extime(otree2, group_id = 7))
})
...
testthat::test_that("extime (e) - this group_id not there", {
  ...
  testthat::expect_error(extime(otree2, group_id = 47))
})
...
testthat::test_that("App time - all apps, seconds = FALSE", {
  ...
  # Test if the values and the single_durations align
  test5 <- round(output$dictator$min_duration, 2) ==
    round(min(output$dictator$single_durations$duration), 2)
  test6 <- round(output$dictator$max_duration, 2) ==
    round(max(output$dictator$single_durations$duration), 2)
  test7 <- round(output$dictator$mean_duration, 2) ==
    round(mean(output$dictator$single_durations$duration), 2)
  testthat::expect_true(all(c(test1, test2, test3, test4,
                              test5, test6, test7)))
})
...
testthat::test_that("App time - all apps, seconds = TRUE", {
  ...
  # Run function
  output_second <- apptime(otree2, seconds = TRUE, digits = 4)
  output_minutes <- apptime(otree2, seconds = FALSE, digits = 4)

  # Test
  testthat::expect_output(str(output_second), "List of 4")
  test1 <-
    round(output_minutes$dictator$mean_duration * 60, 2) ==
    round(output_second$dictator$mean_duration, 2)
  test2 <-
    round(output_minutes$dictator$min_duration  * 60, 2) ==
    round(output_second$dictator$min_duration, 2)
  test3 <-
    round(output_minutes$dictator$max_duration * 60, 2) ==
    round(output_second$dictator$max_duration, 2)
  testthat::expect_true(all(c(test1, test2, test3)))
})
...
testthat::test_that("App time - all apps, old oTree", {
  ...
  test1 <- grepl("Durations not calculated", output$start$message)
  ...
  test6 <- round(output$dictator$min_duration, 2) ==
    round(min(output$dictator$single_durations$duration), 2)
  test7 <- round(output$dictator$max_duration, 2) ==
    round(max(output$dictator$single_durations$duration), 2)
  test8 <- round(output$dictator$mean_duration, 2) ==
    round(mean(output$dictator$single_durations$duration), 2)
  testthat::expect_true(all(c(test1, test2, test3, test4,
                              test5, test6, test7, test8)))
})
...
testthat::test_that("App time - epoch time ", {
  ...
  # Test if the values and the single_durations align
  test5 <- round(output$dictator$mean_duration, 2) ==
    round(min(output$dictator$single_durations$duration), 2)
  test6 <- round(output$dictator$max_duration, 2) ==
    round(max(output$dictator$single_durations$duration), 2)
  test7 <- round(output$dictator$mean_duration, 2) ==
...
testthat::test_that("App time - all apps old oTree specified person", {
  ...
  otree2 <- otree_2_2_4
  otree2 <- make_ids(otree2)
  person <- otree_2_2_4$all_apps_wide$participant.code[
    otree_2_2_4$all_apps_wide$participant._current_app_name == "survey"
  ][1]
...
testthat::test_that("App time - all apps old oTree group", {
  ...
  # Test for start
  test1 <- setequal(names(output$start), c("first_app_one_page", "message"))
  testthat::expect_true(test1)
...
testthat::test_that("App time - all apps no session info ", {
  ...
  # Test
  testthat::expect_output(str(output), "List of 4")
  test1 <- setequal(names(output), c("chatapp", "dictator", "start", "survey"))
  testthat::expect_true(test1)
...
testthat::test_that("App time - one app: single durations didnt make it", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2 <- make_ids(otree2)
...
testthat::test_that("App time - two apps: single durations didnt make it", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2 <- make_ids(otree2)
  person <- otree2$all_apps_wide$participant.code[
    otree2$all_apps_wide$participant._current_app_name == "dictator"][1]
...
testthat::test_that("App time - all apps: single durations didnt make it", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2 <- make_ids(otree2)
  person <- otree2$all_apps_wide$participant.code[
...
testthat::test_that("App time - one app: several durations didnt make it", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2 <- make_ids(otree2)
...
testthat::test_that("App time - several apps: several didnt make it", {
  ...
  testthat::expect_true(sum1 == sum3)
...
testthat::test_that("App time - one duplicate participant - all data one app", {
  # Prepare data
  otree2 <- otree_5_4_0
  person1 <- unique(otree2$Time$participant_code)[1]
...
testthat::test_that("App time - some duplicate participants", {
  ...
  otree2 <- otree_5_4_0
  person1 <- unique(otree2$Time$participant_code)[1]
  person2 <- unique(otree2$Time$participant_code)[2]
  otree2$Time <- rbind(otree2$Time,
                       otree2$Time[otree2$Time$participant_code == person1, ],
...
testthat::test_that("App time - participant is there more often ", {
  # Prepare data
  otree2 <- otree_5_4_0
  persons <- unique(otree2$Time$participant_code)[c(1, 2, 3, 4)]
...
testthat::test_that("App time - group", {
  ...
  # Run function
  output <- apptime(otree2, group_id = 2, apps = "dictator")
...
testthat::test_that("App time - one app durations not calculated", {
  ...
  # Test
  test1 <- setequal(names(output$survey), c("message"))
...
testthat::test_that("App time - participant label", {
  ...
  person_code <- otree2$all_apps_wide$participant.code[
    otree2$all_apps_wide$participant._current_app_name == "survey" &
      otree2$all_apps_wide$participant.label != ""][1]
...
testthat::test_that("App time (e) - no group_id", {
  ...
  # Run function and test
  testthat::expect_error(
    apptime(otree2, group_id = 6, apps = "dictator"),
...
testthat::test_that("App time (e) - participant.label and no all_apps_wide", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$all_apps_wide <- NULL
...
testthat::test_that("pagesec", {
  ...
  test2 <- mean(otree2$Time$seconds_on_page2,
                na.rm = TRUE) > 0
  test3 <- all(
    !is.na(otree2$Time$minutes_on_page) ==
      (!is.na(otree2$Time$seconds_on_page2) / 60))
...
testthat::test_that("pagesec - minutes ", {
  # Run function
  otree2 <- pagesec(otree_5_4_0, minutes = TRUE)
  otree2 <- pagesec(otree_5_4_0, minutes = FALSE)

  # Test
  variable <- otree2$Time$seconds_on_page2
  test1 <- mean(variable, na.rm = TRUE) > 0
...
testthat::test_that("pagesec (e) - no epoch time", {
  ...
  testthat::expect_error(
    ...
    "no variable referring to the epoch time")
})
...
testthat::test_that("Show constant - empty ", {
  ...
  test1 <- length(output$all_apps_wide) == 0
  test2 <- length(output$chatapp) == 0
  test3 <- length(output$dictator) == 0
  test4 <- length(output$start) == 0
  test5 <- length(output$survey) == 0
  test6 <- length(output$Time) == 0
  test7 <- length(output$Chats) == 0
  testthat::expect_true(all(c(test1, test2, test3, test4,
                              test5, test6, test7)))
})
...
testthat::test_that("Show constant - -99", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$all_apps_wide$constant_col <- -99 # Make fake column

  # Run function
  output <- show_constant(otree2, value = -99)
  ...
  test1 <- length(output$all_apps_wide) == 1
  test2 <- length(output$chatapp) == 0
  test3 <- length(output$dictator) == 0
  test4 <- length(output$start) == 0
  test5 <- length(output$survey) == 0
...
testthat::test_that("Assign variable (e) - aaw not there", {
  ...
  otree2 <- assignv_to_aaw(otree2,
                           ...
                           variable = "younger30",
                           newvar = "younger30",
                           resafter = "survey.1.player.age"), "There is no")
})
...
testthat::test_that("Assign variable to aaw", {
  ...
  otree2$survey$younger30 <-
    ifelse(otree2$survey$player.age < 30, 0, 1)
...
testthat::test_that("Assign variable to aaw - one person missing", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$survey$player.age[1] <- 15 # Make sure the first player has a value
  person <- otree2$survey$participant.code[1]
  otree2$survey$younger30 <-
    ifelse(otree2$survey$player.age < 30, 0, 1)
  otree2$survey <- otree2$survey[c(3:nrow(otree2$survey)), ]
...
testthat::test_that("Assign variable to aaw (w)", {
  ...
  # Create new variable
  person <- otree2$survey$participant.code[1]
  otree2$survey <- otree2$survey[c(2:nrow(otree2$survey)), ] # Delete first row
  otree2$survey$younger30 <- ifelse(otree2$survey$player.age < 30, 0, 1)
...
testthat::test_that("Assign variable to aaw (e) - only one var and newvar", {
  # Prepare data
  otree2 <- otree_5_4_0
  otree2$survey$younger30 <- ifelse(otree2$survey$player.age < 30, 0, 1)
  otree2$survey$younger30
